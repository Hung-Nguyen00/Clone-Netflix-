...
@WebServlet("/ToggleDislikeController")
public class ToggleDislikeController extends HttpServlet {
    private static final long serialVersionUID = 1L;
    Activity_HistoryDAO dao = new Activity_HistoryDAO();
    public ToggleDislikeController() {
        super();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        	...
        	try {
        		if (dao.checkActivity(account_id, movie_id)==false) {
        			...
        			dao.toggleDislike(account_id, movie_id);
				}else {
					dao.toggleDislike(account_id, movie_id);
				}
